	...
	render: =>
		...
		h("div.post", {key: @row.key, enterAnimation: Animation.slideDown, exitAnimation: Animation.slideUp, animate_scrollfix: true, classes: {selected: @row.selected}, style: @css_style}, [
			...
			if @meta
				@meta.render()
			h("div.actions", [
				h("a.icon.link", {href: "#Comment", onclick: @handleCommentClick}, h("i.fa.fa-comment.icon-comment"), "Comment"),
				h("a.icon.link", {classes: {active: Page.user?.likes[post_uri], loading: @submitting_like, "like-zero": @row.likes == 0}, href: "#Like", onclick: @handleLikeClick},
